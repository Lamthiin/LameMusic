// music-frontend/src/utils/api.js
 ...
 *
 * FIX:
 * - token key phải là 'accessToken' (đúng với loginApi)
 * - không dùng BASE_URL (chưa khai báo) -> dùng API_BASE_URL hoặc dùng luôn api instance
 */
export const getListenHistoryApi = async (limit = 100) => {
  ...
  try {
    // dùng api instance cho đồng bộ baseURL + interceptor
    const response = await api.get(`/history/me?limit=${limit}`);
    return response.data;
  } catch (error) {
