...
const filteredHidden = albumsHidden.filter(a => {
  ...
  const totalPagesHidden = Math.ceil(filteredHidden.length / itemsPerPage) || 1;

  const showingAlbumsAll = filteredAll.slice(
    (pageAll - 1) * itemsPerPage,
    (pageAll - 1) * itemsPerPage + itemsPerPage
  );

  const showingAlbumsHidden = filteredHidden.slice(
  ...
  const openEdit = async (album) => {
    try {
      const res = await axios.get(
        `http://localhost:3000/admin/albums/${album.id}`
      );
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Bạn chắc chắn muốn xóa album này (chuyển vào thùng rác)?"
      )
    )
      return;
    try {
      const res = await axios.patch(
        `http://localhost:3000/admin/albums/${id}/soft-delete`
      );
    ...
    <div className="album-management">
      ...
      <div className="am-search-box">
        ...
        <input
          ...
          placeholder={
            tab === "all"
              ? "Tìm album trong tất cả album..."
              : "Tìm album trong album bị xóa..."
          }
          value={search}
          onChange={(e) => {
          ...
          <>
            <AlbumHiddenList
              ...
              onRestore={async (id) => {
                if (
                  !window.confirm(
                    "Bạn chắc chắn muốn khôi phục album này?"
                  )
                )
                  return;
                try {
                  const res = await axios.patch(
                    `http://localhost:3000/admin/albums/${id}/restore`
                  );
                  alert(res.data.message);
                  loadAlbums();
                  loadHiddenAlbums();
