...
export class AdminAlbumService {
  ...
  async findAvailableSongs(albumId: number) {
    ...
    // ⛔ Trước đây join 'song.artist' → không còn phù hợp mô hình collab
    // ✅ Dùng song.songArtists -> artist
    const songs = await this.songRepo
      ...
      .andWhere('artist.id = :artistId', { artistId }) // thuộc đúng nghệ sĩ của album (dù solo hay collab)
      .orderBy('song.title', 'ASC')
      .getMany();

    // 3. Format dữ liệu trả ra
    return songs.map((s) => {
      const primaryArtist = s.songArtists?.[0]?.artist;
      ...
      return {
        ...
        artist_name: primaryArtist?.stage_name ?? '—',
  ...
  async findOne(id: number) {
    const album = await this.albumRepo.findOne({
      where: { id },
      // ⛔ Trước: ['artist', 'songs']
      // ✅ Với collab cần thêm songArtists + artist để lấy tên nghệ sĩ
      relations: ['artist', 'songs', 'songs.songArtists', 'songs.songArtists.artist'],
    });
      ...
      songs_list:
        album.songs?.map((s) => {
          ...
          return {
            ...
            artist_name: primaryArtist?.stage_name ?? null,
          };
        }) ?? [],
    });
  }
  ...
  async findFull(id: number) {
    const album = await this.albumRepo.findOne({
      where: { id },
      // ⛔ Trước: ['artist', 'songs', 'songs.artist']
      // ✅ Đổi sang collab: songs.songArtists.artist
      relations: ['artist', 'songs', 'songs.songArtists', 'songs.songArtists.artist'],
    });
  ...
  async update(id: number, dto: any, cover?: Express.Multer.File) {
    ...
    album.release_date = dto.release_date ? new Date(dto.release_date) : album.release_date;
